...
<li class='active'>{$Language->translate('Health Tips')}</li>";

$PageTitle = $Language->translate('Health Tips');
...
?><script src="Editor/ckeditor/ckeditor.js"></script>
        ...
        <div class="page-header">
            <div>
                <button type="button" class="btn btn-success pull-right"
                        onclick='showAddNewForm("<?php echo $Language->translate("Add New Health Tips"); ?>","<?php echo BASE_URL; ?>HealthTips/Form",1000,1000)'>
                    <small class="glyphicon glyphicon-plus-sign"></small>
                    <?php echo $Language->translate("Add New Health Tips"); ?>
                </button>
            </div>
    ...
    <script>
        $(function () {
            ...
            $('#menu-HealthTips').addClass('active');
            ...
            $('#HealthTipsList').ajaxGrid({
                ...
                url: '<?php echo BASE_URL; ?>HealthTips/List',
                requestType: 'get',
                ...
                postContent: [
                    {
                        control: $('<button name ="Edit Heath News" type="button" class="btn btn-rounded btn-info" onclick=\'showEditForm(this,"<?php echo $Language->translate("Edit Health News"); ?>","<?php echo BASE_URL; ?>HealthTips/Form",1000,1000)\'>' +
                                '<small class="glyphicon glyphicon-pencil"></small>' +
                                '</button>')
                    },
                    {
                        control: $("<form style='display: inline-block' action='<?php echo BASE_URL; ?>HealthTips/Delete' method='POST'>" +
                                "<input type='hidden' name='ID' id='ID' /> " +
                                '<button name="Delete HealthTips" type="submit" class="btn btn-rounded btn-danger" onclick=\'return Confirmation(this,"<?php echo $Language->translate("Delete Health News"); ?>","<?php echo $Language->translate("Are you sure you want to delete?"); ?>", "<?php echo $Language->translate("Yes"); ?>", "<?php echo $Language->translate("No"); ?>")\'>' +
