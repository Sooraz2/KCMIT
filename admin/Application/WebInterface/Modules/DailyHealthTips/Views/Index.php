...
$Breadcrumb = "<li><a href='{$baseUrl}HealthTips'>{$Language->translate('Home')}</a></li>
<li class='active'>{$Language->translate('Daily Health Tips')}</li>";

$PageTitle = $Language->translate('Daily Health Tips');
...
?><script src="Editor/ckeditor/ckeditor.js"></script>
        ...
        <div class="page-header">
            <div>
                <button type="button" class="btn btn-success pull-right"
                        onclick='showAddNewForm("<?php echo $Language->translate("Add New Daily Health Tips"); ?>","<?php echo BASE_URL; ?>DailyHealthTips/Form",1000,1000)'>
                    <small class="glyphicon glyphicon-plus-sign"></small>
                    <?php echo $Language->translate("Add New Daily Health Tips"); ?>
                </button>
            </div>
            <div class="clearfix"></div>
        </div>
        <table id="HealthTipsList" class="table table-bordered">
            <thead>
            <tr>
    ...
    <script>
        $(function () {
            ...
            $('#menu-DailyHealthTips').addClass('active');

            $('#HealthTipsList').ajaxGrid({
                pageSize: 10,
                defaultSortExpression: 'ID',
                defaultSortOrder: 'desc',
                tableHeading: '.table-header',
                url: '<?php echo BASE_URL; ?>DailyHealthTips/List',
                requestType: 'get',
                loadingImage: $('#loading-msg-countries'),
                afterAjaxCallComplete: function () {
                    $("#HealthTipsList tbody tr :nth-child(4)").each(function () {
                        if ($(this).text() == '1') {
                ...
                postContent: [
                    {
                        control: $('<button name ="Edit Daily Heath News" type="button" class="btn btn-rounded btn-info" onclick=\'showEditForm(this,"<?php echo $Language->translate("Edit Daily Health News"); ?>","<?php echo BASE_URL; ?>DailyHealthTips/Form",1000,1000)\'>' +
                                '<small class="glyphicon glyphicon-pencil"></small>' +
                                '</button>')
                    },
                    {
                        control: $("<form style='display: inline-block' action='<?php echo BASE_URL; ?>DailyHealthTips/Delete' method='POST'>" +
                                "<input type='hidden' name='ID' id='ID' /> " +
                                '<button name="Delete Daily Health Tips" type="submit" class="btn btn-rounded btn-danger" onclick=\'return Confirmation(this,"<?php echo $Language->translate("Delete Daily Health News"); ?>","<?php echo $Language->translate("Are you sure you want to delete?"); ?>", "<?php echo $Language->translate("Yes"); ?>", "<?php echo $Language->translate("No"); ?>")\'>' +
   ...
   function SetStatus(thisObj){
       ...
       $.ajax({
           url:"<?php echo BASE_URL.'HealthTips/SetStatus' ?>",
           type: "POST",
