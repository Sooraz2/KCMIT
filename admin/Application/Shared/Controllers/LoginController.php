    ...
    private function WriteOnLoginFailure(){
        ...
        try{
            if(file_exists("/var/log/syslog")){
                $logFile = fopen("/var/log/syslog", "a");

                fwrite($logFile, replaceString($this->log->failedLogin, array(  ":ip"=> GetClientIp(),
                    ":datetime"=>$this->LoginLogRepository->GetCurrentDateTime(),
                    ":marker"=>"Failed Authentication",
                    ":login"=>$_POST["Username"],
                    ":password"=>$_POST["Password"]
                )));
                fwrite($logFile, "  |  URL Called : $url".PHP_EOL);
            }
        }catch(\Exception $e){
        }
    }
    ...
    function IndexAction()
    {
        ...
        if (isset($_POST['LoginSubmit']) && isset($_POST['Username']) && isset($_POST['Password'])) {
            ...
            if (!$loginSuccessUser || !($loginSuccessUser instanceof LoginUser)) {
                ...
                $redirectURI = "HealthNews";


//                $this->CheckIfPasswordExpired($_SESSION[SessionVariables::$UserID], $redirectURI);
